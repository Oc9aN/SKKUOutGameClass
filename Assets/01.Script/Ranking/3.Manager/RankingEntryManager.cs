...
public class RankingEntryManager : MonoBehaviour
{
    public static RankingEntryManager Instance;

    private RankingEntryRepository _entryRepository;
    ...
    {
        // 저장소 초기화
        _entryRepository = new RankingEntryRepository();  // :contentReference[oaicite:0]{index=0}
        _entryRepository.AddTestRankingData(RankingType.KillPoint);
    ...
    {
        List<RankingEntryDTO> entries = _entryRepository.LoadRankingList(rankingType);
        return entries;
    }
    ...
    public void Save()
    {
        ...
        // 저장소에 저장
        _entryRepository.SaveRanking(_rankingEntry);  // :contentReference[oaicite:3]{index=3}
